...
import { updateWorkspace, getWorkspaceById } from "@/lib/workspace";
...
export default function CreateWorkspacePage() {
      ...
      const fetchWorkspaceDetails = async () => {
        try {
            ...
            if (workspace) {
                ...
                setMembers(workspace.members.map((member: any) => ({ name: member.name, email: member.email })));
            }
        } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const resp = await updateWorkspace(workspaceId as string, title, description, members.map(member => member.email));
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <div className="flex flex-col gap-2 mt-3">
              {members.filter(member => member.email !== session.data?.user?.email).map((member, idx) => (
                <div
                  key={member.email}
